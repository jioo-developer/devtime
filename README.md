...
| 동작 | API·흐름 | 구체적 절차 |
|------|----------|-------------|
| **POST** (업로드) | 1) `POST /api/file/presigned-url`<br>2) S3로 파일 PUT | 1) 서버에 `{ fileName, contentType }` 보내서 presigned URL과 `key` 받기.<br>2) 받은 URL로 `ApiClient.uploadImageToS3(url, file, contentType)` 호출해 S3에 파일 업로드.<br>3) 받은 `key`를 프로필 생성/수정 시 `profileImage` 값으로 넣어 사용. |
| **GET** (조회) | `GET /api/profile` 후 URL 만들기 | 1) 프로필 API 응답의 `profile.profileImage`는 **파일 경로(key)** 뿐이고, 전체 URL이 아님.<br>2) 화면에 보여줄 때 `getProfileImageUrl(key)`로 `NEXT_PUBLIC_S3_IMAGE_BASE_URL` + key를 이어 붙여 최종 URL 생성.<br>3) `<img src={getProfileImageUrl(profile.profileImage)} />` 처럼 사용하면 브라우저가 그 URL로 이미지를 GET 요청해서 불러옴. |
| **PUT** (변경) | 새 이미지 업로드 후 프로필 수정 | 1) 새 이미지를 선택하면, 위 **POST**와 동일하게 presigned URL 발급 → S3에 PUT → 새 `key` 받기.<br>2) `PUT /api/profile` 요청 body에 `profileImage: 새 key` 를 넣어서 프로필 수정.<br>3) 이후 프로필 조회 시 `profileImage`가 새 key로 바뀌어 있어서 새 이미지가 보임. |
